...
qc_CNAqc <- function (run, dir.battenberg, subdir.callsubclones, subdir.postprocessing, 
	subdir.dpclust, subdir.vafpeaks, filename.samplelist, filename.config, 
	filename.chr.sizes, filename.qc, maxN) 
{
	# load libraries
	library(stringr)
	...
	print(ids[1:6])
 	...
 	# BA made edits made below to use paths to subdir in work dir 
	
	filenames.battenberg.segs <- sapply(ids, function(id) file.path(subdir.postprocessing, pasteu(samplelist[id, "tumour_sample_platekey"], "subclones.txt")))
	
	#filenames.battenberg.purity <- sapply(ids, function(id) file.path(subdir.callsubclones, pasteu(samplelist[id, "tumour_sample_platekey"], "cellularity_ploidy.txt")))
	filenames.battenberg.purity <- sapply(ids, function(id) file.path(subdir.callsubclones, pasteu(samplelist[id, "tumour_sample_platekey"], "purity_ploidy.txt")))
	
	filenames.battenberg.plot <- sapply(ids, function(id) file.path(subdir.callsubclones, pasteu(samplelist[id, "tumour_sample_platekey"], "BattenbergProfile_average.pdf")))
	
	filenames.dpclust.optima <- sapply(ids, function(id) file.path(subdir.dpclust, pasteu(samplelist[id, "tumour_sample_platekey"], "optimaInfo.txt")))
	
	filenames.peaks <- sapply(ids, function(id) file.path(subdir.vafpeaks, pastedot(samplelist[id, "participant_id"], id, "peak_data.RData")))
			
	log.message("reading in sample-level data")
	# get IDs for samples that are being QC'd
	...
	if (run > 1) ids <- ids[which(qc[ids, pasteu(paste0("run", run - 1), "filter_overallfilter")] %in% c("FAIL", "FLAG"))] # identify samples not passing previous run
	...
	for (id in ids) {
		...
		segs.conditions$copynumber11 <- segs.auto[
		  (segs.auto$nMaj1_A == 1 & segs.auto$nMin1_A == 1) &
			segs.auto$frac1_A == 1,] # copy number is 1:1 and there is no evidence of sub-clonal change
	
		segs.conditions$copynumber10 <- segs.auto[
		  ((segs.auto$nMaj1_A == 1 & segs.auto$nMin1_A == 0) | (segs.auto$nMaj1_A == 0 & segs.auto$nMin1_A == 1)) &
			segs.auto$frac1_A == 1,] # copy number is 1:0 and there is no evidence of sub-clonal change
	
		segs.auto2 = segs.auto
		segs.auto2[is.na(segs.auto2)] = 0
		segs.conditions$minorCNzero <- segs.auto2[(which(round((segs.auto2$nMin1_A*segs.auto2$frac1_A)+(segs.auto2$nMin2_A*segs.auto2$frac2_A))==0)),]
		...
		segs.conditions$copynumber21 <- segs.auto[
		  ((segs.auto$nMaj1_A == 2 & segs.auto$nMin1_A == 1) | (segs.auto$nMaj1_A == 1 & segs.auto$nMin1_A == 2)) &
			segs.auto$frac1_A == 1,] # copy number is 2:1 and there is no evidence of sub-clonal change
	
		segs.conditions$copynumber22 <- segs.auto[
		  (segs.auto$nMaj1_A == 2 & segs.auto$nMin1_A == 2) &
			segs.auto$frac1_A == 1,] # copy number is 2:2 and there is no evidence of sub-clonal change
	
		segs.conditions$copynumber32 <- segs.auto[
		  ((segs.auto$nMaj1_A == 3 & segs.auto$nMin1_A == 2) | (segs.auto$nMaj1_A == 2 & segs.auto$nMin1_A == 3)) &
			segs.auto$frac1_A == 1,] # copy number is 3:2 and there is no evidence of sub-clonal change
		...
		segs.conditions$homodelall <- segs.auto.tmp[
		  segs.auto.tmp$ctClone1 == 0 |
			sapply(as.double(segs.auto.tmp$ctClone2), identical, 0.0),] # homozygous deletion of either major or (if evidence of sub-clonality > 20%) minor clone
												  
		segs.conditions$homodelallclonal <- segs.auto.tmp[
		  which(segs.auto.tmp$nMaj1_A == 0 & segs.auto.tmp$nMin1_A==0 & segs.auto.tmp$frac1_A == 1),] # clonal homdels	 
		...
		segs.conditions$homodelallsubclonal <- segs.auto.tmp[
		  ...
		  (segs.auto.tmp$nMaj2_A == 0 & segs.auto.tmp$nMin2_A==0)),] # subclonal homdels (only those > 20%)	
		...
		segs.conditions$cnodd <- segs.auto[
		  (((segs.auto$nMaj1_A == 2 & segs.auto$nMin1_A == 1) | (segs.auto$nMaj1_A == 1 & segs.auto$nMin1_A == 2)) |
			...
			segs.auto$frac1_A == 1,] # copy number is 1:0, 2:1, 3:0, 5:0, 4:1, 3:2, 3:1, 5:1, or 5:3 and there is no evidence of sub-clonal change

		segs.conditions$aroundpoint5.narrow <- segs.auto[
		segs.auto$frac1_A <= thres.50pcpeak.upper & segs.auto$frac1_A >= thres.50pcpeak.lower,] # copy number fraction around 0.5, narrow boundary
	
		segs.conditions$aroundpoint5.wide <- segs.auto[
		segs.auto$frac1_A <= thres.50pcpeak.upper.wide & segs.auto$frac1_A >= thres.50pcpeak.lower.wide,] # copy number fraction around 0.5, wide boundary
		...
		# if it's the last run (4 was original default for last run, now set by maxN), use DPClust params, unless the sample has switched ploidy twice, in which case use VAFPeaks
		...
		if (qc[id, pasteu(run.name, "ploidy_type_accepted")]=="FAIL" | qc[id, pasteu(run.name, "ploidy_type_accepted")]=="FLAG") {
		  ...
		  qc[id, pasteu(run.name, "reestimated_ploidy")] <- qc[id, pasteu(run.name, "dpclust_ploidy")]
					 ...
					 run == maxN - 1) { # if ploidy passes, and we're on the last run, overwrite vafpeaks params with DPClust parameters
		...
		if (abs(qc[id, pasteu(run.name, "reestimated_purity")] - qc[id, pasteu(run.name, "battenberg_purity")]) < 0.05 &
			...
			if (!is.na(vafpurity) && vafpurity==T) {
					# check if dpclust purity differs from current purity estimate by more than 5%, if so, use it, if not, mark sample as having no new possible params
